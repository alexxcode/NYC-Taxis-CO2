...
* **`Ramiro Falcon ` _(Data Analyst)_**   [![linkedin](https://img.shields.io/badge/linkedin-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ramiro-falcon/)
...
- Finalmente. El modelo de predicción fue desplegado en Fast-API, por medio de una instancia virtual en Google Cloud. Puede accederse a ella desde "http://34.125.39.205:8000/docs"  (es necesario asegurarse de que el navegador no cambia el protocolo a https, si lo hace la api no va a cargar). Link Directo [aquí](http://34.125.39.205:8000/docs) (aunque se recomienda copiar y pegar la dirección proporcionada en el navegador). El despliegue tiene un End Point para consultar los resultados del modelo para el Pronostico, el ahorro y la diferencia entre estos para emisiones de Co2, emisiones de ruido y ahorro para 12 meses desde el último dato procesado (2023-6). Debido a que las instancias en google tiene un costo, no se garantiza el funcionamiento del link anterior; sin embargo, también fue desplegado en render, y puede consultarse [aquí](https://api-ml-sqfx.onrender.com/docs), se recuerda también que, si el sitio no está activo en los últimos 15 minutos, éste dejará de operar, por lo cual, es probable al intentar acceder a la API la página esté caida, pedimos al usuario que vuelva a intentar 5 minutos después, que es el tiempo en el cual es posible garantizar que el servicio estará en línea de nuevo. Finalmente, el despliegue local puede hacerse utilizando [estos archivos](Sprint-03/Despliegue_ML). 
...
[dashboard](https://drive.google.com/uc?export=download&id=1YTjaLOI16Zkg6-Et5BGCju80QSL_gC7A)
